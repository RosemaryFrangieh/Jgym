...
export default function ClassMemberModal({ member, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const duration = formData.subscription_type === 'daily' ? 1 : 30
    ...
    if (member) {
      ...
      payload.membership_status = 'new'
      payload.renewal_count = 0
      result = await supabase.from('class_members').insert([payload])
    }
    ...
    setSavedMember({ ...payload, class_type: formData.class_type })
  }
      ...
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
        <div className="bg-navy-800 rounded-xl w-full max-w-md p-6 border border-navy-700">
          <div className="flex flex-col items-center text-center mb-6">
            ...
            <h3 className="text-xl font-bold text-white">Class Member Saved</h3>
            <p className="text-slate-400 text-sm mt-1 capitalize">
              {savedMember.class_type} - {savedMember.first_name} {savedMember.last_name} — ${Number(savedMember.amount_paid).toFixed(2)} paid
            </p>
          </div>
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
      <div className="bg-navy-800 rounded-xl w-full max-w-lg p-6 border border-navy-700 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-white">{member ? 'Edit Class Member' : 'Add New Class Member'}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Class Type Selector */}
          <div>
            <label className="block text-sm text-slate-400 mb-1">Class Type</label>
            <div className="grid grid-cols-2 gap-3">
              {['aerobics', 'zumba'].map(t => (
                <button
                  key={t}
                  type="button"
                  name="class_type"
                  ...
                  className={`px-4 py-2.5 rounded-lg font-medium capitalize border transition-colors ${
                    formData.class_type === t
                      ? t === 'aerobics' ? 'bg-purple-500/20 text-purple-300 border-purple-500/50'
                                          : 'bg-pink-500/20 text-pink-300 border-pink-500/50'
                      : 'bg-navy-900 text-slate-400 border-navy-700 hover:border-slate-500'
                  }`}
                >
                  {t}
                </button>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input 
                ...
                required 
                disabled 
                className="w-full bg-navy-900 border border-navy-700 rounded-lg px-3 py-2 text-white disabled:opacity-50" 
              />
            </div>
            <div>
          ...
          <div>
            ...
            <textarea 
              ...
              onChange={handleChange} 
              rows={3} 
              placeholder="Any notes about this class member..." 
              className="w-full bg-navy-900 border border-navy-700 rounded-lg px-3 py-2 text-white resize-none placeholder:text-slate-600" 
            />
          </div>
